...
namespace NHST.Controllers
{
    public class MainOrderController
    {
        ...
        public static double GetTotalPriceVND(int status, string PriceType, string searchtext, int Type, int orderType, string fd, string td )
        {
            var sql = @"select total=SUM(CAST(" + PriceType + " as float)) ";
            sql += "FROM    dbo.tbl_MainOder AS mo LEFT OUTER JOIN ";
            sql += "dbo.tbl_Account AS u ON mo.UID = u.ID LEFT OUTER JOIN ";
        ...
        public static double GetCountStatus(int UID, int status, int orderType)
        {
            var sql = @"select count(ID) as total ";
            sql += "from tbl_MainOder ";
            sql += "where UID=" + UID.ToString();
            if(status != -1)
            {
                sql += " AND STATUS=" + status.ToString();
            }
            
            sql+= " And OrderType=" + orderType.ToString();
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            double total = 0;
            while (reader.Read())
            {
        ...
        public static int GetTotalForOrderListOfDK(int RoleID, int StaffID, int orderType, string searchtext, int Type, string fd, string td, string priceFrom, string priceTo, string st, bool isNotCode, string mvd, string mdh)
        {
            ...
            if (!string.IsNullOrEmpty(searchtext))
            {
                ...
                if (Type == 1)
                    sql += " And mo.ID like N'%" + searchtext + "%'";
                if (Type == 4)
                    sql += " And u.Username = N'" + searchtext + "'";
            }
            if (!string.IsNullOrEmpty(st))
            {
                if (st != "-1")
