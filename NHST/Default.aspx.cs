    ...
    public partial class Default8 : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            if (services.Count > 0)
            {
                foreach (var item in services)
                {
                    ...
                    ltrService.Text += "<a href=\"" + item.ServiceLink + "\" class=\"services-box\">";
                    ltrService.Text += "<div class=\"icon\">";
                    ltrService.Text += "<img src=\"" + item.ServiceIMG + "\" alt=\"\">";
                    ltrService.Text += "</div>";
                    ltrService.Text += "<div class=\"text\"><p>" + item.ServiceName + "</p></div>";
                    ltrService.Text += "</a>";
                }
            }
            ...
            if (steps.Count > 0)
            {
                ...
                foreach (var item in steps)
                {
                    ...
                    ltrStep1.Text += "<a href=\"" + item.StepLink + "\" class=\"cot\">";
                    ltrStep1.Text += "<div class=\"box\">";
                    ltrStep1.Text += "<div class=\"images-box\">";
                    ltrStep1.Text += "<img src=\"" + item.StepIMG + "\" alt=\"\">";
                    ltrStep1.Text += "</div>";
                    ltrStep1.Text += "<div class=\"text-box\"><p>" + item.StepContent + "</p></div>";
                    ltrStep1.Text += "</div>";
                    ltrStep1.Text += "</a>";
        ...
        public void SearchPage(string page, string text)
        {
            ...
            else if (page == "mercari")
            {
                ...
                linkgo = "https://www.mercari.com/jp/search/?keyword=" + textsearch_mercari + "&type=p&vmarket=&spm=875.7931836%2FB.a2227oh.d100&from=mallfp..pc_1_searchbutton";
            }
            ...
            else if (page == "paypay")
            {
                string a = text;
                string textsearch_shopping = GetHashString(a);
                linkgo = "https://paypaymall.yahoo.co.jp/search?p=" + textsearch_shopping + "&sc_i=pmall_pc_top_hsrch&cid=&brandid=&kspec=&catopn=&b=1";
            }
        ...
        public static string getInfo(string ordecode)
        {
            ...
            if (smallpackage != null)
            {
                ...
                {
                    returnString += "<li class=\"clear\">";
                    returnString += "  Mã vận đơn chưa về kho JP";
                    returnString += "</li>";
                }
                else if (smallpackage.Status == 2)
                {
                    ...
                    returnString += "      <p class=\"tit\"><span class=\"grey89 font-w\">Trạng thái:</span><span class=\"m-color\"> Đã về kho JP</span></p>";
        ...
        [WebMethod]
        public static string getCurrency(string totalprice, string giasp, string weight)
        {

            TinhTienChoCaoThang a = new TinhTienChoCaoThang();
            ...
            if (config != null)
            {
                pc_config = Convert.ToDouble(config.Currency);
            }
            double pricepro = 0;
            double servicefee = 0;
            if (Convert.ToDouble(giasp) > 0)
                pricepro = Convert.ToDouble(giasp) * pc_config;
            ...
            double totalweight = Math.Round(Convert.ToDouble(weight), 3);
            double feeweight = 0;

            var fee = WarehouseFeeController.GetByAndWarehouseFromAndToWarehouseAndShippingTypeAndAndHelpMoving(1, 1, 1, false);
            if (fee.Count > 0)
            {
                foreach (var f in fee)
                {
                    ...
                    {
                        feeweight = totalweight * Convert.ToDouble(f.Price);
                    }
                }
            }
            ...
            a.TyGia = pc_config;
            a.FeeWeight = Math.Round(feeweight,0);
            a.FeeOrder = Math.Round(FeeOrder, 0);
            JavaScriptSerializer serializer = new JavaScriptSerializer();
        ...
        public class TinhTienChoCaoThang
        {
            public double FeeOrder { get; set; }
