...
namespace NHST.manager
{
    public partial class OutStock1 : System.Web.UI.Page
    {
                    ...
                    if (acc != null)
                    {
                        ...
                        if (kq.ToInt(0) > 0)
                        {
                            ...
                            for (int i = 0; i < packs.Length - 1; i++)
                            {
                                ...
                                if (small != null)
                                    OutStockSessionPackageController.Insert(ousID, small.ID, small.OrderTransactionCode, Convert.ToInt32(small.MainOrderID), Convert.ToInt32(small.TransportationOrderID), currentDate, username_current);
        ...
        protected void btn_ExportExcel(object sender, EventArgs e)
        {
            var Username = ddlUsername.SelectedValue;
            int type = 0;// ddlStatus.SelectedValue.ToInt(0);
            int orderID = txtOrderID.Text.ToInt(0);

            List<PackageGet> pgs = new List<PackageGet>();

            DateTime currentDate = DateTime.Now;
            var account = AccountController.GetByID(Username.ToInt(0));
            if (account != null)
            {
            ...
            if (pgs.Count > 0)
            {
                string strFile = "danh-sach-ton-kho-" + Username + "_" + DateTime.Now.ToShortDateString() + ".xlsx";
                ...
                dt.Columns.Add("Loại đơn hàng", typeof(string));
                dt.Columns.Add("Mã vận đơn", typeof(string));
                dt.Columns.Add("Cân nặng", typeof(string));
                dt.Columns.Add("Kích thước", typeof(string));
                dt.Columns.Add("Tổng lưu kho", typeof(string));
                ...
                foreach (var item in pgs)
                {
                    ...
                    try
                    {
                        ...
                        var Order = (item.mID > 0 ? item.mID : item.tID);
                        dt.Rows.Add(
                                 Order.ToString(), item.username, item.OrderTypeString, item.barcode, item.weight, item.dai + "x" + item.rong + "x" + item.cao,
                                 item.TotalDayInWarehouse
                ...
                //Name of File  
                //string fileName = "DanhSachDaDuyetTheoNam.xlsx";
                using (XLWorkbook wb = new XLWorkbook())
                {
                    //Add DataTable in worksheet  
                    var ws = wb.Worksheets.Add(dt);
                    //Remove Themes
                    wb.Worksheet(1).Tables.FirstOrDefault().Theme = XLTableTheme.None;
                    //Remove Auto Filter (Header)
                    wb.Worksheet(1).Tables.FirstOrDefault().ShowAutoFilter = false;

                    //Style
                    wb.Worksheet(1).Row(1).Style.Font.Bold = true;
                    wb.Worksheet(1).Row(1).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                    wb.Worksheet(1).Style.Font.FontName = "Times New Roman";
                    wb.Worksheet(1).Columns("A", "V").AdjustToContents();
